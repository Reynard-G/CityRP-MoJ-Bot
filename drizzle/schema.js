const { mysqlTable, int, timestamp, smallint, varchar, decimal, customType } = require("drizzle-orm/mysql-core");
...
const criminalEvidence = mysqlTable("criminalEvidence", {
	...
	indictment_id: int("indictment_id").notNull().references(() => criminalIndictments.id, { onDelete: "restrict", onUpdate: "cascade" }),
	attachment: customMediumBlob("attachment").notNull(),
	updated_at: timestamp("updated_at", { mode: 'string' }).default('current_timestamp()').notNull(),
...
const criminalIndictments = mysqlTable("criminalIndictments", {
	id: int("id").autoincrement().notNull(),
	criminal_id: smallint("criminal_id").notNull().references(() => criminals.id, { onDelete: "restrict", onUpdate: "cascade" }),
